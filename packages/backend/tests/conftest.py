...
def stub_fetch_prices(monkeypatch):
    ...
    def mock_yfinance_provider_init(self, config, fallback_providers=None):
        self.config = config
        self.fallback_providers = fallback_providers
        self.cache = MagicMock() # Mock cache manager
        self.max_retries = config.YFINANCE_MAX_RETRIES
        self.backoff_factor = config.YFINANCE_BACKOFF_FACTOR
        self.timeout = config.YFINANCE_TIMEOUT
