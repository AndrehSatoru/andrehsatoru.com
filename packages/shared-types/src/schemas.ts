...
const MonteCarloRequest = z
  .object({
    ...
    vol_method: z.enum(["std", "ewma", "garch"])
      .optional()
      .default("std"),
    ewma_lambda: z.number().gte(0.5).lte(0.999).optional().default(0.94),
...
const FFBetaPlotRequest = z
  .object({
    ...
    start_date: z.string(),
    end_date: z.string(),
    rf_source: z.enum(["ff", "selic", "us10y"])
      .optional()
      .default("selic"),
...
const ComprehensiveChartsRequest = z
  .object({
    ...
    plot_configs:
      z.union([z.object({}).partial().passthrough(), z.null()]).optional(),
  })
  .passthrough();
...
const AssetAllocationRequest = z
  .object({
    weights: z.record(z.number()),
    title:
      z.union([z.string(), z.null()]).optional().default("Alocação de Ativos"),
  })
  .passthrough();
const CumulativePerformanceRequest = z
  .object({
    ...
    title:
      z.union([z.string(), z.null()]).optional().default("Performance Acumulada"),
  })
  .passthrough();
...
const DashboardRequest = z
  .object({
    ...
    dashboard_type:
      z.enum(["portfolio", "risk", "performance"])
        .optional()
        .default("portfolio"),
  })
  .passthrough();
...
- **risk_free_rate**: Taxa livre de risco anualizada (ex: 0.05 para 5%). 
  Se não especificada, usa o valor configurado em 
x60;RISK_FREE_RATE
x60;.`,
    requestFormat: "json",
    parameters: [
