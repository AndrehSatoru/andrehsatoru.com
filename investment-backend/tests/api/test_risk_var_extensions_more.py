...
def test_ivar_std_and_ewma(monkeypatch_prices):
    ...
    # std
    r1 = client.post("/risk/ivar", json={**payload_base, "method": "std"})
    assert r1.status_code == 200
    ivar1 = r1.json()["result"]["ivar"]
    assert set(ivar1.keys()) == {"AAA", "BBB"}
    # ewma
    r2 = client.post("/risk/ivar", json={**payload_base, "method": "ewma", "ewma_lambda": 0.94})
...
def test_relvar_methods_std_ewma(monkeypatch_prices, monkeypatch_benchmark):
    ...
    # std
    r1 = client.post("/risk/relvar", json={**base, "method": "std", "benchmark": "^DUMMY"})
    assert r1.status_code == 200
    rel1 = r1.json()["result"]["relative_var"]
    assert isinstance(rel1, float)
    # ewma
    r2 = client.post("/risk/relvar", json={**base, "method": "ewma", "ewma_lambda": 0.9, "benchmark": "^DUMMY"})
    assert r2.status_code == 200
